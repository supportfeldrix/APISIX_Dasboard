...
class EscalationService:
    ...
    async def _start_grace_period(
        ...
        Persists the pending state to the database and creates an asyncio task
        that will escalate when the grace period expires.

        Args:
            rule: The alert rule.
            context: Alert context with failure details.
            db: Database session.
        """
    ...
    async def _grace_period_worker(self, rule_id: int, grace_seconds: float) -> None:
        ...
        Handles CancelledError gracefully (means the task was cancelled via
        acknowledge or recovery).

        Args:
            rule_id: The alert rule ID.
            grace_seconds: Seconds to sleep before escalating.
        """
        try:
    ...
    async def _on_grace_period_expired(self, rule_id: int) -> None:
        ...
        Opens a new DB session since the original session from the alert trigger
        may be closed by now.
    ...
    def _get_effective_grace_period(self, rule: AlertRule, db: Session) -> int:
        ...
        Returns:
            Effective grace period in seconds.
        """
        if rule.critical:
            return 0
    ...
    def _build_failure_message(self, context: AlertContext) -> str:
        ...
        Returns:
            A descriptive failure message string.
        """
        parts = [
    ...
    def _log_event(
        ...
        Args:
            ...
            event_type: The event type (e.g., trigger_file_written, acknowledged).
            db: Database session.
            file_path: Optional file path (for trigger/recovery files).
            username: Optional username (for acknowledgment events).
