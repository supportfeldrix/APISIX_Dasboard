...
class BackgroundScheduler:
    ...
    async def _run_cycle(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # Mark baseline as established after the first full cycle.
                # All routes now have their current metrics stored for delta
    ...
    async def _evaluate_metric_rule(
        ...
        For UPSTREAM_ERROR, uses delta-based evaluation: only alerts when NEW
        5xx errors appear since the last evaluation cycle. Sends recovery when
        no new 5xx errors appear AND new 2xx requests have come in.

        For JWT_FAILURE and HIGH_ERROR_RATE, uses the original cumulative logic.

        The first evaluation cycle after pod startup is used purely to establish
        a baseline — no alerts are fired. This prevents false positives from
        cumulative Prometheus counters that persist across dashboard restarts.
        ...
        # ─── Baseline guard ─────────────────────────────────────────────────
        # On the first evaluation cycle after pod startup, _baseline_established
        # is False. We store metrics for all routes AND seed the high-water marks
        # so that the next cycle (the first real evaluation) does not treat the
        # entire cumulative Prometheus counter as "new" errors.
        if not self._baseline_established:
            metrics = route_metrics.get(route_id)
            if metrics is not None:
                self._previous_metrics[route_id] = dict(metrics)

                # Seed UPSTREAM_ERROR HWM from baseline
                ...
                # Seed CLIENT_ERROR HWM from baseline
